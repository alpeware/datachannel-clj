...
- [ ] **LINT-01:** Tighten `.clj-kondo/config.edn` rules and resolve all codebase warnings. Merge this linter rules your config: `{:linters {:large-def {:level :warning :row-threshold 60}  :too-many-params {:level :warning :max-params 5}  :missing-docstring {:level :warning} :unsorted-required-namespaces {:level :warning}}}`
