  ...
  (testing "Close Connection After Second Lost Heartbeat"
    (let [now 1000000
      ...
      (let [timer-expire-time (+ now 30000)
            {:keys [new-state network-out app-events]} (core/handle-timeout state :t-heartbeat timer-expire-time)]
        ...
        (is (some? (get-in new-state [:timers :t-heartbeat-rtx])) "Should start t-heartbeat-rtx timer")
        ...
        (let [rtx-expire-time (+ timer-expire-time 1000)
              {:keys [new-state network-out app-events]} (core/handle-timeout new-state :t-heartbeat-rtx rtx-expire-time)]
          ...
          (let [timer-expire-time2 (+ timer-expire-time 30000)
                {:keys [new-state network-out app-events]} (core/handle-timeout new-state :t-heartbeat timer-expire-time2)]
            (is (= 1 (count network-out)) "Should send another heartbeat packet")
            (is (= :heartbeat (-> network-out first :chunks first :type)) "Packet should be heartbeat")
            (is (some? (get-in new-state [:timers :t-heartbeat-rtx])) "Should restart t-heartbeat-rtx timer")
            ...
            (let [rtx-expire-time2 (+ timer-expire-time2 1000)
                  {:keys [new-state network-out app-events]} (core/handle-timeout new-state :t-heartbeat-rtx rtx-expire-time2)]
              ...
              (is (nil? (get-in new-state [:timers :t-heartbeat])) "Heartbeat timer should be removed")
              (is (nil? (get-in new-state [:timers :t-heartbeat-rtx])) "Heartbeat RTX timer should be removed"))))))))
