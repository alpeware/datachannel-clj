  ...
  (testing "Send A Lot Of Bytes Missed Second Packet"
    (let [now-ms 1000
          ...
          packets (:network-out res-a4)
            _ (is (> (count packets) 1) "A should send multiple packets due to MTU fragmentation")

            ;; Z receives the first packet
            res-z3 (@#'core/handle-sctp-packet state-z2 (first packets) now-ms)
            state-z3 (:new-state res-z3)
            sack-pkt-1 (first (:network-out res-z3))

            ;; A receives the first SACK
            res-a5 (@#'core/handle-sctp-packet state-a4 sack-pkt-1 now-ms)
            state-a5 (:new-state res-a5)

            ;; Z *misses* the second packet

            ;; Z receives the third packet
            res-z4 (@#'core/handle-sctp-packet state-z3 (nth packets 2) now-ms)
            state-z4 (:new-state res-z4)
            sack-packets (:network-out res-z4)
            ;; Z should send a SACK indicating the gap
            _ (is (seq sack-packets) "Z should send a SACK packet")
            _ (is (= :sack (:type (first (:chunks (first sack-packets))))) "Z should have a SACK chunk")
            _ (is (= [[2 2]] (:gap-blocks (first (:chunks (first sack-packets))))) "Z should report the gap correctly")

            ;; A receives the SACK packet with gap
            res-a6 (@#'core/handle-sctp-packet state-a5 (first sack-packets) now-ms)
            state-a6 (:new-state res-a6)

            ;; A should eventually timeout and retransmit the missing packet
            timeout-ms (+ now-ms 2000) ;; After T3-RTX expires
            res-a7 (@#'core/handle-timeout state-a6 :sctp/t3-rtx timeout-ms)
            _state-a7 (:new-state res-a7)
            rtx-packets (:network-out res-a7)
            _ (is (seq rtx-packets) "A should retransmit the missing packet")
            ...
            event-payload (:payload (first (filter #(= :on-message (:type %)) app-events)))]
